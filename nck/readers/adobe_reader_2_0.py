...
@processor("adobe_api_key","adobe_tech_account_id","adobe_org_id","adobe_client_secret","adobe_metascopes","adobe_private_key_path")
...
class AdobeReader_2_0(Reader):
    ...
    def __init__(
        ...
        self.jwt_client = JWTClient(api_key, tech_account_id, org_id, client_secret, metascopes, private_key_path)
    ...
    def build_report_description(self,breakdown_item_ids,metrics):
        ...
        rep_desc = {
                ...
                "metricContainer": {},
                "dimension": "variables/{}".format(self.dimensions[len(breakdown_item_ids)]),
    ...
    def get_report_page(self,rep_desc,page_nb=0):
        ...
        response = requests.post(
            "https://analytics.adobe.io/api/{}/reports".format(self.jwt_client.global_company_id),
    ...
    def get_parsed_report(self,rep_desc,metrics,parent_dim_parsed):
        ...
        first_response = self.get_report_page(rep_desc=rep_desc,page_nb=0)
        ...
        if first_response['totalPages']>1:
            for page_nb in range(1,first_response['totalPages']):
                next_response = reader.get_report_page(rep_desc=rep_desc,page_nb=page_nb)
    ...
    def get_node_values(self,breakdown_item_ids):
        ...
        rep_desc = self.build_report_description(breakdown_item_ids=breakdown_item_ids,metrics=["visits"])
        first_response = self.get_report_page(rep_desc=rep_desc,page_nb=0)
        node_values = get_node_values_from_response(first_response)

        if first_response['totalPages']>1:
            for page_nb in range(1,first_response['totalPages']):
                node_values += get_node_values_from_response(reader.get_report_page(rep_desc=rep_desc,page_nb=page_nb))
    ...
    def read(self,graph=None,node=None):
        """
        ...
        if graph:
            ...
            if len(path_to_node) == len(self.dimensions)-1:
                ...
                breakdown_item_ids = get_item_ids_from_nodes(path_to_node)
                rep_desc = self.build_report_description(breakdown_item_ids=breakdown_item_ids,metrics=self.metrics)
                data = self.get_parsed_report(rep_desc=rep_desc,metrics=self.metrics,parent_dim_parsed=parent_dim_parsed)

                def result_generator():
                    yield from data
                    
                yield JSONStream("adobe_results", result_generator())

        else:
            # Create graph and add first level of nodes
            graph, node, path_to_node, visited = {}, "root", [], []
            graph = self.add_child_nodes_to_graph(graph=graph,node=node,path_to_node=path_to_node)
        ...
        for child_node in unvisited_childs:
            ...
            yield from self.read(graph=graph,node=child_node)
