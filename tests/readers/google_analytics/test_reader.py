...
class GoogleAnalyticsReaderTest(TestCase):
    ...
    def test_format_date(self):
        ...
        assert GoogleAnalyticsReader.format_date(test_date) == "2019-01-01"
        self.assertRaises(ValueError, GoogleAnalyticsReader.format_date, wrong_date)

    def test_get_days_delta(self):
        inputs = ["PREVIOUS_DAY", "LAST_7_DAYS", "LAST_30_DAYS", "LAST_90_DAYS"]
        expected = [1, 7, 30, 90]
        output = [GoogleAnalyticsReader.get_days_delta(input) for input in inputs]
    ...
    def test_read(self, mock_query):
        ...
        reader = GoogleAnalyticsReader(**kwargs)
        kwargs["add_view"] = True
        reader_with_view_id = GoogleAnalyticsReader(**kwargs)
