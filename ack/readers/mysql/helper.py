...
def build_table_query(engine, schema, table, watermark_column, watermark_value):
    if watermark_column and watermark_value:
        return _build_table_query_with_watermark(engine, schema, table, watermark_column, watermark_value)
    else:
        return _build_table_query_without_watermark(engine, schema, table)
...
def _build_table_query_without_watermark(engine, schema, table):
    return _get_table(engine, schema, table).select()


def _build_table_query_with_watermark(engine, schema, table, watermark_column, watermark_value):
    t = _get_table(engine, schema, table)
    return t.select().where(t.columns[watermark_column] > watermark_value)


def _get_table(engine, schema, table):
    meta = _get_meta(engine, schema)
    table = sqlalchemy.Table(table, meta, autoload=True, autoload_with=engine)

    return table


def _get_meta(engine, schema):
    global _engine_meta
