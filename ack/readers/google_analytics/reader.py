...
from ack.readers.google_analytics.config import DATEFORMAT, DISCOVERY_URI, GOOGLE_TOKEN_URI, PREFIX
from ack.readers.reader import Reader
from ack.streams.json_stream import JSONStream
from ack.utils.retry import retry
from ack.utils.text import strip_prefix
    ...
    def format_and_yield(self, view_id, report):
        ...
        for row in report["data"].get("rows", []):
            ...
            if "date" in formatted_response:
                formatted_response["date"] = GoogleAnalyticsReader.format_date(formatted_response["date"])
